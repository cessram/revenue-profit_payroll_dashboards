...
# Clear cache on reload during development
...
def load_data(path: str = None, uploaded_file=None) -> pd.DataFrame:
    ...
    # Numeric columns - convert to numeric
    numeric_cols = [
        "Cost",  # hourly cost rate (from CSV)
        "Actual_Cost",  # total cost from your R script
        "Billable Hours",
        "Billed Hours",
        "Non Billed Hours",
        "Hourly Billing Rate",
    ...
    # Use existing Revenue column (already in CSV)
    if "Revenue" not in df.columns:
        df["Revenue"] = 0.0

    # IMPORTANT: use Actual_Cost as the "Cost" used in all dashboard calcs
    # so totals match your R code: sum(Actual_Cost)
    if "Actual_Cost" in df.columns:
        df["Cost"] = df["Actual_Cost"]

    # Total hours per row (only Billed Hours in this dataset)
    if "Billed Hours" in df.columns:
        df["Total Hours"] = df["Billed Hours"].fillna(0)

    # Profit: if CSV already has Profit, keep it; otherwise compute from Revenue - Actual_Cost
...
# -------------------------
# SIDEBAR – FILE UPLOAD & DATA LOADING
# -------------------------
st.sidebar.title("🎯 Dashboard Controls")
st.sidebar.markdown("### 📁 Data Source")
...
with st.spinner("Loading data..."):
    if uploaded_file is not None:
        ...
        df_raw = load_data(path=DATA_PATH)
...
view_mode = view_mode.split(" ", 1)[1]  # Remove emoji
...
# Status filter: All / Billable / Not Billable
status_mode = st.sidebar.selectbox(
    "Status Filter",
    ["All", "Billable only", "Not Billable only"],
    index=1,  # default = Billable only
)
...
# For Not Billable view, treat Non Billable Hours as the Billed Hours (numeric)
if (
    status_mode == "Not Billable only"
    and "Non Billable Hours" in df_filtered.columns
):
    df_filtered["Billed Hours"] = (
        pd.to_numeric(df_filtered["Non Billable Hours"], errors="coerce")
        .fillna(0)
    )
...
if "Project Category" in df_filtered.columns:
    ...
    selected_proj_cats = st.sidebar.multiselect(
        "Project Category",
        options=proj_cats,
        default=proj_cats,
    )
...
def aggregate_profitability(df: pd.DataFrame, group_col: str, min_hours_filter: float = 0.0) -> pd.DataFrame:
    ...
    # 🔒 Force key columns to numeric to avoid 'str' > 0 errors
    ...
    agg["Revenue_per_Hour"] = np.where(
        agg["Billable_Hours"] > 0,
        agg["Revenue"] / agg["Billable_Hours"],
        0
    )
    agg["Cost_per_Hour"] = np.where(
        agg["Billable_Hours"] > 0,
        agg["Cost"] / agg["Billable_Hours"],
        0
    )
    agg["Profit"] = agg["Revenue"] - agg["Cost"]
    agg["Profit_Margin"] = np.where(
        agg["Revenue"] > 0,
        agg["Profit"] / agg["Revenue"],
        0
    )
    agg["Profit_per_Hour"] = np.where(
        agg["Billable_Hours"] > 0,
        agg["Profit"] / agg["Billable_Hours"],
        0
    )
    agg["Cost_per_Hour"] = np.where(
        agg["Billable_Hours"] > 0,
...
def show_global_kpis(df: pd.DataFrame, show_comparison=False):
    ...
    # --- make sure key columns are numeric *here* ---
    ...
    col1.metric(
        "💰 Total Revenue",
        f"${total_rev:,.0f}",
        help="Total revenue from all work in the current filter"
    )
    col2.metric(
        "💵 Total Profit",
        f"${profit:,.0f}",
    ...
    col3.metric(
        "⏱️ Billed Hours",
        f"{total_hours:,.0f}",
        help="Total Billed Hours in the current filter"
    )
    col4.metric(
        "📊 Revenue/Hour",
        f"${rev_per_hour:.2f}",
        help="Average revenue per billed hour"
    )
    col5.metric(
        "💵 Avg Rate",
        f"${avg_rate:.2f}/hr" if avg_rate > 0 else "N/A",
        help="Average billing rate"
    )
...
if view_mode == "Projects / Clients":
    ...
    # Group level selector with better layout
    col1, col2 = st.columns([2, 1])
    with col1:
        # Only show grouping options that exist in the data
    ...
    # CRITICAL: Stop immediately if aggregation is empty
    if agg is None or len(agg) == 0 or agg.empty:
        st.error("❌ AGGREGATION FAILED - No data returned")
        with st.expander("🔍 Debug Information"):
            st.write(f"**Input data shape:** {df_filtered.shape}")
            st.write(f"**Group column:** {group_level}")
            st.write(f"**Min hours filter:** {min_hours}")
            st.write(f"**Aggregation result type:** {type(agg)}")
            st.write(f"**Aggregation is None:** {agg is None}")
            st.write(f"**Aggregation length:** {len(agg) if agg is not None else 'N/A'}")
            if agg is not None and not agg.empty:
                st.write("**Aggregation columns:**", list(agg.columns))
    ...
    if agg.empty:
        ...
        with st.expander("🔍 Diagnostic Information"):
            ...
            st.write(f"**Grouping by:** {group_level}")
            st.write(
                f"**Available {group_level} values:** {df_filtered[group_level].nunique() if group_level in df_filtered.columns else 'Column not found'}")
            if group_level in df_filtered.columns:
                st.write(f"**Sample {group_level} values:**", df_filtered[group_level].unique()[:10].tolist())
            st.write(
                f"**Total Billed Hours in filtered data:** {df_filtered['Billed Hours'].sum() if 'Billed Hours' in df_filtered.columns else 'N/A'}")
        st.stop()
    ...
    if len(agg) == 0:
        st.error("❌ Aggregation returned empty dataframe")
        st.stop()

    # Main visualization
    y_col = "Profit_Margin" if profit_metric_choice == "Profit Margin (%)" else "Profit_per_Hour"

    # Debug: verify agg has data and required columns
    if len(agg) == 0:
        st.error("❌ Aggregation dataframe is empty!")
        st.stop()

    required_cols = ["Revenue_per_Hour", y_col, "Billable_Hours", group_level]
    # Filter out None values
    required_cols = [c for c in required_cols if c is not None]
    missing = [c for c in required_cols if c not in agg.columns]

    if missing:
        st.error(f"❌ Missing columns in aggregation: {', '.join(str(m) for m in missing)}")
        st.write("**Available columns:**", list(agg.columns))
        st.write("**Sample data:**")
        st.dataframe(agg.head())
        st.stop()

    if chart_type == "Scatter":
        # ABSOLUTE SAFETY CHECK - DO NOT REMOVE
        if len(agg) == 0:
            st.error("🛑 CRITICAL: Cannot create scatter plot - aggregation is empty!")
            st.stop()
        ...
        fig_scatter = px.scatter(
            ...
            labels={
                "Revenue_per_Hour": "Revenue per Hour ($/hr)",
                "Profit_Margin": "Profit Margin",
                "Profit_per_Hour": "Profit per Hour ($/hr)",
        ...
        fig_bar = px.bar(
            ...
            title=f"Top 20 {group_level} by Revenue",
            template="plotly_white",
    ...
    fig_rev = px.bar(
        ...
        title=f"Top {top_n} by Revenue/Hour",
        template="plotly_white",
    ...
    fig_prof = px.bar(
        ...
        title=f"Top {top_n} by Profit/Hour",
        template="plotly_white",
        color="Profit_per_Hour",
    ...
    fig_margin = px.bar(
        ...
        title=f"Top {top_n} by Profit Margin",
        template="plotly_white",
        color="Profit_Margin",
        color_continuous_scale="RdYlGn"
    )
    ...
    selected_group = st.selectbox(
        f"Select a {group_level} for detailed analysis",
    ...
    if not detail_df.empty:
        ...
        if "YearMonth" in detail_df.columns:
            ...
            fig_trend.update_layout(
                title=f"Monthly Trend for {selected_group}",
                xaxis_title="Month",
                yaxis_title="Amount ($)",
                template="plotly_white",
                height=300
        ...
        cols_to_show = [
            "Worked Date", "Client Name", "Project Name", "Project Category",
            "Task or Ticket Title", "Resource Name", "Billed Hours",
            "Hourly Billing Rate", "Revenue", "Cost", "Profit", "Status (Billable / Non-Billable)"
        ]
    ...
    with col1:
        st.download_button(
            ...
            file_name=f"{group_level.replace(' ', '_').lower()}_profitability.csv",
            mime="text/csv",
        )
    with col2:
        st.download_button(
            f"📥 Download {selected_group} Details",
            detail_df.to_csv(index=False).encode("utf-8"),
            file_name=f"{selected_group.replace(' ', '_')}_details.csv",
            mime="text/csv",
        )
...
# -------------------------
    ...
    if "Total Hours" in df_filtered.columns and "Billed Hours" in df_filtered.columns:
        ...
        agg_cons = agg_cons.merge(hours_by_consultant[group_col],
                                  on=group_col, how="left")

        if "Total Hours" in hours_by_consultant.columns:
            agg_cons = agg_cons.merge(
                hours_by_consultant[[group_col, "Total Hours"]],
                on=group_col,
                how="left",
                suffixes=('', '_total')
            )
            agg_cons["Utilization"] = np.where(
                agg_cons["Total Hours"] > 0,
                (agg_cons["Billable_Hours"] / agg_cons["Total Hours"]) * 100,
                0
            )
    else:
    ...
    fig_scatter_cons = px.scatter(
        ...
        title=f"Consultant Performance: Revenue vs Profit per Hour",
        template="plotly_white"
    )
    ...
    # NOT BILLABLE REVENUE DETAIL & BILLING CODE CHART
    # -----------------------------------------
    if status_mode == "Not Billable only":
        st.markdown("---")
        st.markdown("### 🧾 Not Billable – Revenue by Billing Code")

        nb_df = df_filtered.copy()  # already filtered to Not Billable

        if nb_df.empty:
            st.info("No Not Billable records for the current filters.")
        else:
            # Make sure we have Billing Code column
            if "Billing Code Name" in nb_df.columns:
                # Aggregate revenue by Billing Code Name
                nb_agg = (
                    nb_df.groupby("Billing Code Name", dropna=False)
                ...
                # KPI for total Not Billable revenue
                st.metric(
                    "Total Not Billable Revenue",
                    f"${total_nb_rev:,.0f}",
                    help="Sum of Revenue for all Not Billable entries under current filters",
                )

                # Bar chart: Revenue by Billing Code
                fig_nb = px.bar(
                    nb_agg.sort_values("Revenue", ascending=True),
                    y="Billing Code Name",
                    x="Revenue",
                    orientation="h",
    ...
    fig_cons_util = px.bar(
        top_util_cons,
        x=group_col,
        y="Utilization",
        title="By Utilization",
        labels={"Utilization": "%"},
        template="plotly_white",
        color="Utilization",
...
# -------------------------
    ...
    display_cols = [
        group_col, "Revenue", "Cost", "Profit", "Profit_Margin",
        "Billable_Hours", "Revenue_per_Hour", "Profit_per_Hour",
        "Utilization", "Avg_Billing_Rate", "ROI"
    ]
    ...
    st.markdown("### 💾 Export Data")
    st.download_button(
        "📥 Download Consultant Performance Report",
        agg_cons.to_csv(index=False).encode("utf-8"),
        file_name=f"consultant_performance_{group_col.replace(' ', '_').lower()}.csv",
        mime="text/csv",
    )
...
# -------------------------
    ...
    with col2:
        # Build breakdown options based on available columns
    ...
    ts_agg = (
        df_ts.groupby(group_cols, as_index=False)
        .agg(
            Revenue=("Revenue", "sum"),
            Cost=("Cost", "sum"),
            Billable_Hours=("Billed Hours", "sum"),
    ...
    ts_agg["Profit_Margin"] = np.where(
        ts_agg["Revenue"] > 0,
        ts_agg["Profit"] / ts_agg["Revenue"],
        np.nan
    )
    ts_agg["Revenue_per_Hour"] = np.where(
        ts_agg["Billable_Hours"] > 0,
        ts_agg["Revenue"] / ts_agg["Billable_Hours"],
        np.nan
    )
    ...
    if breakdown == "Overall":
        ...
        fig.update_layout(
            title=f"{time_grain} Revenue, Profit & Cost Trend",
            xaxis_title="Period",
            yaxis_title="Amount ($)",
            template="plotly_white",
            height=500,
        ...
        with col1:
            fig_hours = px.bar(
                ...
                x=time_col,
                y="Billable_Hours",
                title=f"{time_grain} Billed Hours",
        ...
        with col2:
            fig_margin = px.line(
                ...
                x=time_col,
                y="Profit_Margin",
                title=f"{time_grain} Profit Margin",
                labels={"Profit_Margin": "Margin"},
                template="plotly_white"
            )
            fig_margin.update_yaxes(tickformat=".1%")
        ...
        if len(ts_agg) > 1:
            ...
            revenue_growth = ((last_period["Revenue"] - first_period["Revenue"]) /
                              first_period["Revenue"] * 100) if first_period["Revenue"] > 0 else 0
            profit_growth = ((last_period["Profit"] - first_period["Profit"]) /
                             first_period["Profit"] * 100) if first_period["Profit"] > 0 else 0
            hours_growth = ((last_period["Billable_Hours"] - first_period["Billable_Hours"]) /
                            first_period["Billable_Hours"] * 100) if first_period["Billable_Hours"] > 0 else 0
        ...
        selected_entities = st.multiselect(
            f"Select {breakdown}(s) to compare",
            options=entities,
    ...
    st.download_button(
        "📥 Download Time Series Report",
        ts_agg.to_csv(index=False).encode("utf-8"),
